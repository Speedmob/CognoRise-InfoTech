...
frame1 = Frame(root)
frame1.pack()
titleLabel = Label(frame1 , text="Tic Tac Toe" , font=("Arial" , 26) , bg="cyan" , width=16 )
...
def restartGame():
    global buttons, turn, board, game_end
    game_end = False
...
def play(event):
    ...
    if button["text"] == " ":
        if turn == "x" :
            ...
            updateBoard()
            if checkForWin(turn):
                winningLabel = Label(frame1 , text=f"{turn} wins the game" , bg="cyan", font=("Arial" , 26),width=16)
                winningLabel.grid(row = 0 , column=0 , columnspan=3)
                game_end = True
            turn = "x"
        ...
        if checkForDraw():
            drawLabel = Label(frame1 , text=f"Game Draw" , bg="cyan", font=("Arial" , 26), width = 16)
...
restartButton = Button(frame2 , text="Restart Game" , width=19 , height=1 , font=("Arial" , 20) , bg="cyan" , relief=RAISED , borderwidth=5 , command=restartGame )
